...
const tourSchema = z.object({
  ...
  availableSeats: z.number().positive({ message: "Количество доступных мест должно быть положительным числом" }),
  nextTourDate: z.string().optional(),
});

// GET запрос для получения списка туров менеджером
export async function GET(req: Request) {
  ...
  try {
    ...
    // Получаем туры с пагинацией и включаем все необходимые поля
...
// POST запрос для создания нового тура менеджером
